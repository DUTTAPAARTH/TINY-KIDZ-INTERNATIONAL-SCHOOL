...
async function navigateToFees(page) {
  ...
  await page.click('a:has-text("Fees"), button:has-text("Fees")');
...
test.describe('Fee Module - Complete Button & Form Testing', () => {
  ...
  test.describe('Tab Navigation', () => {
    test('should display all 5 tabs', async ({ page }) => {
      ...
      // Verify tab labels
      await expect(page.locator('[role="tab"]:has-text("Fee Structure")')).toBeVisible();
      await expect(page.locator('[role="tab"]:has-text("Generate Fees")')).toBeVisible();
      await expect(page.locator('[role="tab"]:has-text("View Records")')).toBeVisible();
      await expect(page.locator('[role="tab"]:has-text("Defaulters")')).toBeVisible();
      await expect(page.locator('[role="tab"]:has-text("Reports")')).toBeVisible();
    });

    test('should switch between tabs correctly', async ({ page }) => {
      // Fee Structure tab
      await page.click('[role="tab"]:has-text("Fee Structure")');
      await expect(page.locator('h5:has-text("Fee Structure")')).toBeVisible({ timeout: 5000 });
      ...
      await expect(page.locator('h5:has-text("Fee Records")')).toBeVisible({ timeout: 5000 });

      // Defaulters tab
      await page.click('[role="tab"]:has-text("Defaulters")');
      await expect(page.locator('h5:has-text("Defaulters")')).toBeVisible({ timeout: 5000 });

      // Reports tab
      await page.click('[role="tab"]:has-text("Reports")');
      await expect(page.locator('[class*="FeeReports"], text="Reports",h5:has-text("Daily")')).toBeVisible({ timeout: 5000 });
    });
  });
  ...
  test.describe('Tab 0: Fee Structure - Buttons & Forms', () => {
    ...
    test('should display Fee Structure tab with required elements', async ({ page }) => {
      ...
      await expect(page.locator('label:has-text("Academic Year")')).toBeVisible();
      const yearOptions = await page.locator('[role="listbox"] [role="option"]').count();
      expect(yearOptions).toBeGreaterThan(0);

      // Check for Set Fee Structure button
      const setButton = page.locator('button:has-text("Set Fee Structure")');
    ...
    test('should open Set Fee Structure dialog', async ({ page }) => {
      ...
      await expect(page.locator('text="Set Fee Structure"')).toBeVisible();

      // Verify dialog contains expected fields
      await expect(page.locator('label:has-text("Class")')).toBeVisible();
      await expect(page.locator('label:has-text("Academic Year")')).toBeVisible();
      await expect(page.locator('label:has-text("Tuition Fee")')).toBeVisible();
    });
    ...
    test('should display edit and delete buttons for structures', async ({ page }) => {
      ...
      if (rowCount > 0) {
        ...
        const editButton = page.locator('button [svg]').filter({ hasText: 'Edit' }).first();
        await expect(editButton).toBeVisible();

        // Check for delete button
        const deleteButton = page.locator('button [svg]').filter({ hasText: 'Delete' }).first();
        await expect(deleteButton).toBeVisible();
      }
    });

    test('should change academic year filter', async ({ page }) => {
      const yearSelect = page.locator('label:has-text("Academic Year")').locator('..').locator('[role="button"]');
  ...
  test.describe('Tab 1: Generate Fees - All 4 Buttons & Forms', () => {
    ...
    test('should display all 4 generate buttons', async ({ page }) => {
      ...
      await expect(page.locator('h6:has-text("Generate for One Class")')).toBeVisible();
      await expect(page.locator('h6:has-text("Generate for Whole School")')).toBeVisible();
      await expect(page.locator('h6:has-text("Generate for Class Range")')).toBeVisible();
      await expect(page.locator('h6:has-text("Generate for One Student")')).toBeVisible();
    });

    test('Button 1: Generate for One Class - open dialog', async ({ page }) => {
      // Find the Configure button under "Generate for One Class"
      const classCard = page.locator('h6:has-text("Generate for One Class")').locator('..').locator('..');
      const configButton = classCard.locator('button:has-text("Configure")');
      await configButton.click();

      await expect(page.locator('[role="dialog"]:has-text("Generate Fees for One Class")')).toBeVisible({ timeout: 3000 });
    });
    ...
    test('Button 1: Generate for One Class - fill form', async ({ page }) => {
      ...
      // Select class
      const classSelect = page.locator('[role="dialog"] label:has-text("Class")').locator('..').locator('[role="button"]');
      await classSelect.click();
    ...
    test('Button 2: Generate for Whole School - open dialog', async ({ page }) => {
      const schoolCard = page.locator('h6:has-text("Generate for Whole School")').locator('..').locator('..');
      const configButton = schoolCard.locator('button:has-text("Configure")');
      await configButton.click();

      await expect(page.locator('[role="dialog"]:has-text("Generate Fees for Whole School")')).toBeVisible({ timeout: 3000 });
    });
    ...
    test('Button 2: Generate for Whole School - fill form', async ({ page }) => {
      ...
      const yearSelect = page.locator('[role="dialog"] label:has-text("Academic Year")').locator('..').locator('[role="button"]');
      await yearSelect.click();
    ...
    test('Button 3: Generate for Class Range - open dialog', async ({ page }) => {
      const rangeCard = page.locator('h6:has-text("Generate for Class Range")').locator('..').locator('..');
      const configButton = rangeCard.locator('button:has-text("Configure")');
      await configButton.click();

      await expect(page.locator('[role="dialog"]:has-text("Generate Fees for Class Range")')).toBeVisible({ timeout: 3000 });
    });
    ...
    test('Button 3: Generate for Class Range - fill form', async ({ page }) => {
      ...
      // Fill from class
      await page.fill('[role="dialog"] input[type="number"]', '1', { force: true });
    });

    test('Button 4: Generate for One Student - open dialog', async ({ page }) => {
      const studentCard = page.locator('h6:has-text("Generate for One Student")').locator('..').locator('..');
      const configButton = studentCard.locator('button:has-text("Configure")');
      await configButton.click();

      await expect(page.locator('[role="dialog"]:has-text("Generate Fees for One Student")')).toBeVisible({ timeout: 3000 });
    });
    ...
    test('Button 4: Generate for One Student - fill form', async ({ page }) => {
      ...
      // Select student
      const studentSelect = page.locator('[role="dialog"] label:has-text("Student")').locator('..').locator('[role="button"]');
  ...
  test.describe('Tab 2: View Records - Buttons & Filters', () => {
    ...
    test('should display Update Overdue Status button', async ({ page }) => {
      const updateButton = page.locator('button:has-text("Update Overdue Status")');
      await expect(updateButton).toBeVisible();
    });

    test('should handle Class filter dropdown', async ({ page }) => {
      const classSelect = page.locator('label:has-text("Class")').locator('..').locator('[role="button"]');
      await classSelect.click();
    ...
    test('should handle Quarter filter dropdown', async ({ page }) => {
      const quarterSelect = page.locator('label:has-text("Quarter")').locator('..').locator('[role="button"]');
    ...
    test('should handle Fee Type filter dropdown', async ({ page }) => {
      const feeTypeSelect = page.locator('label:has-text("Fee Type")').locator('..').locator('[role="button"]');
  ...
  test.describe('Tab 3: Defaulters - Search & Filters', () => {
    ...
    test('should use Due Filter dropdown', async ({ page }) => {
      const dueSelect = page.locator('label:has-text("Due Filter")').locator('..').locator('[role="button"]');
  ...
  test.describe('UI Responsiveness & Accessibility', () => {
    ...
    test('should display error messages if API fails gracefully', async ({ page, context }) => {
  ...
  test.describe('Fee Module Integration', () => {
    test('should flow through all tabs in sequence', async ({ page }) => {
      const tabs = ['Fee Structure', 'Generate Fees', 'View Records', 'Defaulters', 'Reports'];
      ...
      for (const tabName of tabs) {
        ...
        const tabContent = page.locator(`h5:has-text("${tabName}"), text="${tabName}"`).first();
