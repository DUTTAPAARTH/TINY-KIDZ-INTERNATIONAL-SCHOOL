...
const TeacherHomework = () => {
  ...
  useEffect(() => {
    ...
    fetchHomework(controller.signal);
    return () => controller.abort();
  }, [fetchHomework]);

  useEffect(() => {
    const controller = new AbortController();
    api.get("/teachers/me", { signal: controller.signal }).then(res => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setDialogOpen(false);
      fetchHomework(new AbortController().signal);
    } catch (e) {
      console.error("Save homework error:", e?.response?.data || e.message || e);
      setSnackbar({ open: true, message: e?.response?.data?.message || e?.response?.data?.error || e.message || "Error saving homework", severity: "error" });
    }
  };
  ...
  return (
    <TeacherLayout>
      ...
      {error ? (
        <Box textAlign="center" color="#C62828" mt={4}>
          <Typography variant="h6" gutterBottom>Failed to load homework</Typography>
          <Button variant="outlined" onClick={() => { const c = new AbortController(); fetchHomework(c.signal); }} sx={{ mt: 1 }}>Retry</Button>
        </Box>
      ) : loading ? (
